...
如果你希望手动编写生成器和校验器，可以按照以下步骤操作：

#### 1. 挑选模板

根据题目涉及的数据结构，从 `references/templates/` 中选择合适的模板：

| 数据结构 | 推荐生成器模板 | 推荐校验器模板 |
|---------|---------------|---------------|
| 普通数组/序列 | `gen-array-with-opt.cpp` | `ival.cpp` / `nval.cpp` |
| 普通树 | `gen-tree-graph.cpp` | `undirected-tree-validator.cpp` |
| 有根树 | `gen-rooted-tree-graph.cpp` | `undirected-tree-validator.cpp` |
| 二分图 | `gen-bipartite-graph.cpp` | `bipartite-graph-validator.cpp` |
| 网格图 | `gen-grid-graph.cpp` | `grid-validator.cpp` |
| 一般图 | `gen-basic-fallback.cpp` | `undirected-graph-validator.cpp` |
...
# 复制模板（以树形结构题目为例）
cp ../references/templates/generators/gen-tree-graph.cpp gen.cpp
cp ../references/templates/validators/undirected-tree-validator.cpp valid.cpp
...
int main(int argc, char *argv[]) {
    ...
    // 通过命令行参数获取子任务编号和测试点编号
    int subtask = opt<int>(1);  // 子任务编号
    int testCase = opt<int>(2); // 测试点编号
    
    // 根据子任务设置不同的数据规模
    int n;
    if (subtask == 1) {
        n = rnd.next(1, 100);      // Subtask 1: 小规模
    } else if (subtask == 2) {
        n = rnd.next(1, 1000);     // Subtask 2: 中等规模
    } else {
        n = rnd.next(1, 100000);   // Subtask 3: 大规模
    }
    
    // 生成树结构（使用模板提供的 rnd.perm 和树生成算法）
    vector<int> p(n);
    for (int i = 1; i < n; i++)
        p[i] = rnd.next(i);  // 随机父节点
    
    vector<int> perm = rnd.perm(n);
    
    println(n);
    for (int i = 1; i < n; i++)
        println(perm[i] + 1, perm[p[i]] + 1);
...
```bash
python3 ../scripts/generate.py gen.cpp valid.cpp std.cpp problem.md 4 3 3
```
...
# 解压题目包查看内容
unzip problem_package_*.zip -d problem_extracted/
ls problem_extracted/
```
